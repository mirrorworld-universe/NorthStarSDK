...
describe('Real Integration Tests', () => {
  const PORTAL_PROGRAM_ID = address('5TeWSsjg2gbxCyWVniXeCmwM7UtHTCK7svzJr5xYJzHf');
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      await (rpc as any).requestAirdrop(delegatedAccount.address, 1000000000).send();
  ...
  test('Step 1: Open Session - should create session and fee vault accounts', async () => {
    ...
    const instruction = {
      ...
      accounts: [
        ...
        { address: address('11111111111111111111111111111111'), role: 0 as const },
      ],
      data: PortalProgram.encodeOpenSession({ gridId, ttlSlots: 2000n, feeCap: 1_000_000n }),
    };
    ...
    try {
      await sendAndConfirmTransaction(transaction, { commitment: 'confirmed', skipPreflight: true });
    } catch (e) {
  ...
  test('Step 2: Delegate Account - should create delegation record', async () => {
    ...
    const delegationRecordPDA = await deriveDelegationRecordPDA(delegatedAccount.address);
